...
class Powerup {
  constructor(x, y, type) {
    this.x = x;
    this.y = y;
this.type = type || 'speed';  // 'speed' | 'shield' | 'triple'
  ...
  draw() {
    const color = this.type === 'shield' ? '#3aff6e'
                : this.type === 'triple' ? '#ff3ec8'
                :                          '#00e5ff';
...
p.dead = true;
        if (p.type === 'speed') {
          ...
          if (ship.shieldTime <= 0) ship.shieldTime = SHIELD_TIME;
        } else {
          ship.tripleTime = 5;
        }
      }
